...
class GradleCon:
    ...
    def compile(logger, signed=False, keystore=None):
        ...
        def on_refresh_done():
            GradleCon.compile_app(logger, signed, keystore)

        def refresh_and_compile():
            GradleCon.refresh_dependencies(logger, refresh_done_event)
            refresh_done_event.wait()
            on_refresh_done()

        build_thread = threading.Thread(target=refresh_and_compile)
        build_thread.start()
    '''
    ...
    '''
    @staticmethod
    def refresh_dependencies(logger, done_event):
        def run_gradle_refresh():
            android_project_dir = os.path.normpath(os.path.join(os.path.abspath(__file__), '..', '..', '..', 'android'))
            ...
            gradle_command += ' build --refresh-dependencies'
            ...
            try:
                ...
                print(f"Dependency refresh process finished with exit code {exit_code}.")
                logger.log(f"Dependency refresh process finished with exit code {exit_code}.<br>")

                done_event.set()

            except Exception as e:
                print(f"Error occurred while refreshing the dependencies: {e}")
                logger.log(f"Error occurred while refreshing the dependencies: {e}<br>")

        refresh_thread = threading.Thread(target=run_gradle_refresh)
